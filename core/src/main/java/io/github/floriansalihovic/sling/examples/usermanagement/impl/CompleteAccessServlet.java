...
/**
 * Servlet which provides access to the service which allows setting complete access for a resource by a user(,group or
 * principal).
 *
...
@SlingServlet(methods = {Constants.GET_METHOD},
        ...
        selectors = {Constants.ALL_SELECTOR})
public class CompleteAccessServlet extends AbstractAclServlet {
    ...
    protected void doGet(SlingHttpServletRequest request,
        ...
        if (null != principalName && principalName.length() > 0) {
            userManagerService.resourceAccessibleAndModifiableFor(resourceResolver, resource, principalName);
